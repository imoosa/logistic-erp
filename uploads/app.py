...
def _rollback_customer_session_on_error(exc):
    """
    This is the fix for the error "getting in everywhere": db_router caches one
    SQLAlchemy session per company_id and reuses it across requests. Nothing
    was rolling that session back after a failed commit (an IntegrityError,
    a bad form value, anything), so a single failed write left the session
    stuck mid-transaction. Every subsequent request for that company — on any
    route, including completely unrelated ones like /clients or a purchase or
    manifest page — then hit PendingRollbackError before it could run a single
    query, because SQLAlchemy refuses to do anything with a session until
    someone calls rollback() on it.

    teardown_request runs after every request, success or failure, while the
    request context (and therefore Flask's `session` cookie data) is still
    available. If this request raised, roll back that company's session so
    the next request starts clean instead of inheriting the poison.

    NOTE: this only knows about get_current_company()'s notion of company_id.
    Routes that operate on a *different* company via an explicit token or id
    (e.g. the PDF-token route, or admin routes juggling multiple company ids
    in a loop) are not covered here — each of those should call
    cdb.rollback() in their own except block too.
    """
    if exc is None:
        return
    company_id = get_current_company()
...
def inject_today():
    ...
    return {"subscription_today": date.today()}
...
def order_add():
    ...
    if request.method == "POST":
        ...
        order_id_val = _next_numbered_id(cdb, Order.order_id, "ORD-" + datetime.now().strftime("%Y%m%d") + "-")
        new_order   = Order(
...
def invoice_new():
    ...
    if request.method == "POST":
        ...
        if edit_invoice_id:
            ...
            # Create new invoice
            invoice_id = _next_numbered_id(cdb, Invoice.invoice_id, "CUST-" + datetime.now().strftime("%Y%m%d") + "-", extra_filters=[Invoice.company_id == company_id])
...
def estimate_new():
    ...
    shipper_invoice_id = _next_numbered_id(cdb, Estimate.estimate_id, "SHIP-" + datetime.now().strftime("%Y%m%d") + "-", extra_filters=[Estimate.company_id == company_id])
...
def estimate_new():
    ...
    if request.method == "POST":
        ...
        # Create new
        estimate_id = _next_numbered_id(cdb, Estimate.estimate_id, "SHIP-" + datetime.now().strftime("%Y%m%d") + "-", extra_filters=[Estimate.company_id == company_id])
